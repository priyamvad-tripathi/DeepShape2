...
def load_model(
    ...
    *,
    ...
    Args:
        ...
        args_override: Constructor kwargs merged over the config `args:` block.
    """
    ...
    prefix = str(model_cfg.get("weights_prefix", ""))
    if prefix and not any(k.startswith(prefix) for k in state_dict):
        state_dict = {prefix + k: v for k, v in state_dict.items()}

    args = model_cfg.get("args", {})
    if isinstance(args, DictConfig):
        args = OmegaConf.to_container(args, resolve=True)
    args = {**(args or {}), **(args_override or {})}

    if class_name == "HQS_PnP":
        args.setdefault("defaults", cfg["hqs_hyperparams"])
        args.setdefault("verbose_hp", verbose)
