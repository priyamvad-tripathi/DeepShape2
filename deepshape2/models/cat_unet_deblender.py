...
class Attention_axial(nn.Module):
    ...
    def forward(self, qkv, H, W, mask=None):
        ...
        x = (
            x.transpose(1, 2).contiguous().reshape(-1, self.H_sp * self.W_sp, C)
...
class CATB_axial(nn.Module):
    ...
    def calculate_mask(self, H, W):
        ...
        img_mask_0 = img_mask_0.view(
            1, H // H, H, self.split_size // self.split_size, self.split_size, 1
        )
        img_mask_0 = (
            img_mask_0.permute(0, 1, 3, 2, 4, 5)
            .contiguous()
            .view(-1, H, self.split_size, 1)
        )
        ...
        img_mask_1 = img_mask_1.view(
            1, self.split_size // self.split_size, self.split_size, W // W, W, 1
        )
        img_mask_1 = (
            img_mask_1.permute(0, 1, 3, 2, 4, 5)
            .contiguous()
            .view(-1, self.split_size, W, 1)
        )
    ...
    def forward(self, x, x_size):
        ...
        if self.shift_size > 0:
            ...
            # V-Shift
            qkv_0 = torch.roll(
                qkv[:, :, :, :, : C // 2], shifts=-self.shift_size, dims=3
            )
            qkv_0 = qkv_0.view(3, B, L, C // 2)
            # H-Shift
            qkv_1 = torch.roll(
                qkv[:, :, :, :, C // 2 :], shifts=-self.shift_size, dims=2
            )
            ...
            # V-Rwin
            x1 = (
                self.attns[0](qkv[:, :, :, : C // 2], H, W)
                .view(B, L, C // 2)
                .contiguous()
            )
            # H-Rwin
            x2 = (
                self.attns[1](qkv[:, :, :, C // 2 :], H, W)
                .view(B, L, C // 2)
                .contiguous()
            )
            # Concat
